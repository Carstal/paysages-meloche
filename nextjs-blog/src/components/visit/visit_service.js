...
async function main(){
   ...
   */
  const uri = "mongodb+srv://Mohaned:0000@cluster0.gvkvlw9.mongodb.net/?retryWrites=true&w=majority";
  const mongouri = "mongodb+srv://carstaltari:Pablo__545@iot2-carlo.ijsiznf.mongodb.net/?retryWrites=true&w=majority";
...
async function createOne(client, data){
    const newVisit = Visit('639fa87e22fbb9582bbb4155',[12,21,32,43], new Date("2015-03-25" ), new Date("2015-03-25" ) );
    const data = {
    ...
    const result = await client.db("ECP-CalendarDummy ").collection("dummy-calendar").insertOne(data)
 ...
 async function deleteById(client, id){
    const result = await client.db("ECP-CalendarDummy ").collection("dummy-calendar").deleteOne({"_id": id});
...
async function updateOne(client, listingUpdate){
    ...
    const result = await client.db("ECP-CalendarDummy ").collection("dummy-calendar").findOne({ collection_id: listingUpdate.collection_id });

    if(result) {
        console.log(`Found a listing in connection with the name '${listingUpdate}'`);
        console.log(result);
        //update record
        client.db("ECP-CalendarDummy ").collection("dummy-calendar").updateOne({ collection_id: listingUpdate.collection_id }, { $set: listingUpdate });
        console.log(`Listing updated`);
        //get record with new values
        const newResult = await client.db("ECP-CalendarDummy ").collection("dummy-calendar").findOne({ collection_id: listingUpdate.collection_id });
...
async function findOne(client, listone){
    const result = await client.db("ECP-CalendarDummy ").collection("dummy-calendar").findOne({ collection_id: listone });
 ...
 async function findall(client, listall){
    const cursor = await client.db("ECP-CalendarDummy ").collection("dummy-calendar").find({ collection_id: listall });
